...
class CustomerBilling(models.Model):
    ...
    state = fields.Selection([
        ...
        ('paid', 'Paid'),
        ('cancel', 'Cancelled'),
    ], string='Status', index=True, default='draft', tracking=True, copy=False,
    ...
    ], readonly=False, index=True, change_default=True, default='out_invoice', tracking=True)
    ...
    #                                          store=True)
    payment_move_line_ids = fields.Many2many('account.move.line', string='Payments')

    auto_load = fields.Boolean(string="Auto Load Due Invoice", default=False)

    date_receipt = fields.Date(string='Due Date',default=fields.Date.today())
    ...
    #     payment_lines = []
    #     if self.invoice_ids:
    #         for line in self.invoice_ids[0].line_ids:
    ...
    def onchange_partner_id(self):
        ...
        xxx_ids = self.env['account.move'].search(
            ['|', ('bill_to_id', '=', self.partner_id.id), ('partner_id', '=', self.partner_id.id),
             ('state', '=', 'posted'), ('billing_id', '=', False), ('move_type', '=', 'out_invoice'),
        ...
        if self.move_type == 'out_invoice':
            ...
            if self.auto_load:
                inv_ids = self.env['account.move'].search(
                    [('state', '=', 'posted'), ('move_type', '=', 'in_invoice'), ('billing_id', '=', False),
    ...
    def action_invoice_register_payment(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': len(active_ids) != 1 and self.env.ref('account.view_account_payment_register_form').id,
            'context': ctx,
            'target': 'new',
