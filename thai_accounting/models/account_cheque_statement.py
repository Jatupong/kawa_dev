...
class AccountChequeStatement(models.Model):
    ...
    reverse_transaction_id = fields.Many2one('account.move', string='Reverse Transaction')
    ...
    def action_reject(self):
        view = self.env.ref('thai_accounting.cheque_reject_form').id
    ...
    def reject_cheque(self):
        ...
        if self.move_new_id:
            revers_move = self.move_new_id._reverse_moves()
            revers_move.update({
                'date': self.validate_date
            })
            self.reverse_transaction_id = revers_move
            self.reverse_transaction_id.action_post()
    ...
    def action_validate(self):
        ...
        print ('Step after move line reverse')
    ...
    def cheque_move_line_reverse_get(self):
        ...
        if line_id:
            ...
            if self.type == 'rec':
                ...
                credit =  0.00
                debit = self.amount
            original_account_id = self.journal_id.default_account_id
        ...
        if original_account_id and new_account_id:
            ...
            res.append({
                ...
                'amount_currency': debit if debit > 0 else credit * (-1),
            ...
            res.append({
                ...
                'amount_currency': credit if credit > 0 else debit * (-1),
                'currency_id': currency_id.id,
                'quantity': 1.00,
                'product_id': False,
                'product_uom_id': False,
