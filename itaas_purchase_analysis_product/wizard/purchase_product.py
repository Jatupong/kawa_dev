...
class WizardPurchaseProductReport(models.TransientModel):
    _name = 'wizard.purchase.product.report'
    ...
    def default_get(self, fields_list):
        result = super(WizardPurchaseProductReport, self).default_get(fields_list)
    ...
    def print_excel_report(self):
        ...
        print('datas ', datas)
        return self.env.ref('itaas_purchase_analysis_product.purchase_product_xls').report_action(self, data=datas)


class PurchaseProductReportXls(models.AbstractModel):
    _name = 'report.itaas_purchase_analysis_product.purchase_product_xls'
    _inherit = 'report.report_xlsx.abstract'

    def _get_stock_move(self, date_from, date_to, product_ids, partner_ids):
        ...
        if partner_ids:
            domain_move += [('picking_id.partner_id', 'in', partner_ids)]
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        print('product_ids ', product_ids)
        move_ids = self._get_stock_move(lines.date_from, lines.date_to, product_ids, data['partner_ids'])
        ...
        i_row += 1
        worksheet.merge_range(i_row, i_col, i_row, i_col + 12, 'รายงานประวัติการซื้อ แยกตามสินค้า', for_left)
        i_row += 1
        ...
        for product in product_ids:
            move_by_product = move_ids.filtered(lambda x:x.product_id.id == product.id)
            print('move_by_product', move_by_product)
            i_row += 1
            i_col = 0
            ...
            for line in move_by_product:
                ...
                i_col += 1
                worksheet.write(i_row, i_col, line.picking_id.partner_id.name, for_left)
                i_col += 1
            ...
            i_row += 1
            i_col = 1
            worksheet.merge_range(i_row, i_col, i_row, i_col + 3, 'รวม', for_left)
            i_col += 4
            worksheet.write(i_row, i_col, sum_product_qty, for_right_num_format)
            i_col += 5
            worksheet.write(i_row, i_col, sum_product_subtotal, for_right_num_format)
