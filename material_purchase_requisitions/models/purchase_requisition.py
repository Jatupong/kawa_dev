...
class MaterialPurchaseRequisitionType(models.Model):
    _name = "material.purchase.requisition.type"
...
class MaterialPurchaseRequisition(models.Model):
    ...
    def _get_internal_picking_count(self):
        for picking in self:
            picking_ids = self.env['stock.picking'].search([('custom_requisition_id','=',picking.id)])
            picking.internal_picking_count = len(picking_ids)

    def _get_purchase_order_count(self):
        for po in self:
            po_ids = self.env['purchase.order'].search([('custom_requisition_id', '=', po.id)])
    ...
    type_request_id = fields.Many2one('material.purchase.requisition.type', string='Type Request')
    ...
    date_end = fields.Date(string='Requisition Deadline', readonly=True,
                           help='Last date for the product to be needed', copy=True)
    ...
    def onchange_custom_picking_type_id(self):
        for rec in self:
            if not rec.location_id:
                rec.location_id = rec.custom_picking_type_id.default_location_src_id
            if not rec.dest_location_id:
                rec.dest_location_id = rec.custom_picking_type_id.default_location_dest_id
    ...
    def _prepare_move_vals(self, line=False, stock_id=False):
        product_uom = line.product_id.uom_id
        product_uom_qty = line.uom._compute_quantity(line.qty, product_uom)
        pick_vals = {
            ...
            'origin': self.requisition_id.sequence,
        ...
        for rec in self:
            ...
            if any(line.requisition_type == 'internal' for line in rec.requisition_line_ids):
                ...
                picking_vals = {
                    ...
                    'type_request_id': rec.type_request_id.id,
                    'origin': rec.name,
            ...
            for line in rec.requisition_line_ids:
                if line.requisition_type == 'internal':
                    move_vals = rec._prepare_move_vals(line, stock_id)
